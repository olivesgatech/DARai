...
class MustafaNet2DTCN(nn.Module):
    def __init__(self, num_classes=8, l3_feature_dim=64, window_size=4, num_l3_classes = 48):
        ...
        self.l3_attention = nn.MultiheadAttention(embed_dim=l3_feature_dim, num_heads=4)

        # Final layer to output class predictions
        self.fc_out = nn.Linear(in_features=l3_feature_dim, out_features=num_classes)

        self.fc_reduce = nn.Linear(in_features=9216, out_features=l3_feature_dim)
        self.fc_l3 = nn.Linear(in_features=l3_feature_dim, out_features=num_l3_classes)
        self.fc_l2 = nn.Linear(in_features=l3_feature_dim+num_l3_classes, out_features=num_classes)
    ...
    def forward(self, x, l3_labels, mode='train'):
        ...
        ##################################################################################
        # x.view(B, T, -1): (4, 4, 9216)
        # self.fc_l3 = 
        # x_reduced = self.fc_reduce(x.view(B, T, -1))  # Output shape: [B, T, l3_feature_dim]
        # x_reduced, _ = self.l3_attention(x_reduced, x_reduced, x_reduced)
        # l3_logits = self.fc_l3(x_reduced)  # L3 예측, Output shape: [B, T, num_l3_classes]
        # l3_pred = torch.argmax(l3_logits, dim=-1)  # [B, T], 예측된 L3 레이블 인덱스
        #################################################################################
        ...
        pos_embed_image = self.positional_embedding_image.unsqueeze(1).expand(T, B, -1).contiguous()
        pos_embed_image = pos_embed_image.permute(1, 0, 2).contiguous().unsqueeze(3).expand(B, T, -1, H_new*W_new).contiguous()
        x_with_pos = x + pos_embed_image.to(x.device)
        #####################################################################################
        x_reduced = x_with_pos.contiguous().view(B, T, -1)  # [B, T, 9216]
        x_reduced = self.fc_reduce(x_reduced)  # [B, T, l3_feature_dim]
        x_reduced = rearrange(x_reduced, 'b t c -> t b c')
        x_l3, _ = self.l3_attention(x_reduced, x_reduced, x_reduced) #[576, 8, 64]
        x_l3 = rearrange(x_l3, 't b c -> b t c')
        l3_logits = self.fc_l3(x_l3)
        l3_pred = torch.argmax(l3_logits, dim=-1)
        #####################################################################################
        ...
        l3_embed = self.l3_embed(l3_pred)  # [B, T, 64]
        l3_embed = l3_embed.permute(1, 0, 2)
        pos_embed_l3 = self.positional_embedding_l3.unsqueeze(1).expand(T, B, -1).contiguous()
        ...
        return x, l3_logits, attn_weights
