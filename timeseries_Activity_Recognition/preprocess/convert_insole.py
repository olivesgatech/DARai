...
for folder in folders:
    ...
    for file in files:
        data=np.loadtxt(file, delimiter=",", dtype=float,skiprows=1,usecols=range(1,19)).T
        N=data.shape[1]
        ...
        trial=session
        data=data.T
        new_file_name=f"a{act}_s{subject_name}_t{trial}_insole.mat"
