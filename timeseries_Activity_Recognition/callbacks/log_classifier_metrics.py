...
class LogClassifierMetrics(pl.Callback):
    ...
    def __init__(self, num_classes, metric_names=['accuracy', 'f1-score', 'precision', 'recall'], average='micro'):
        ...
        self.metric_dict = nn.ModuleDict({
            'accuracy':  torchmetrics.Accuracy(num_classes=num_classes ,task='multiclass', average='macro', multidim_average='global'),
            'f1-score':  torchmetrics.Accuracy(num_classes=num_classes,task='multiclass'),
            'precision': torchmetrics.Precision(num_classes=num_classes,task='multiclass', average=average),
            'recall':    torchmetrics.Recall(num_classes=num_classes,task='multiclass', average=average)
        })
        self._reset_state()
